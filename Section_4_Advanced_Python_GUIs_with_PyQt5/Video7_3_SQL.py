...
from Section_4_Advanced_Python_GUIs_with_PyQt5.Designer_code.Video7_11_SQL_final_style_Design import Ui_MainWindow
from Section_4_Advanced_Python_GUIs_with_PyQt5.Video6_Networking_TCPServer_TCPClient import TcpServer, TcpClient, SERVER_PORT
...
class RunDesignerGUI():
    ...
    def __init__(self):
        ...
        #---------------------
        self.create_DB()                                                    # create DB if it does not exist
        self.model = None
        ...
        self.ui.button_SQL_create_db.clicked.connect(self.create_DB)
        #--------------------- 
        self.update_widgets()
        self.widget_actions()
        self.tab2_actions()
    ...
    def sql_tableview_model(self):
        ...
        db.setDatabaseName('PUBLISHERS.db')
        
        tableview = self.ui.tableView
        self.model = QtSql.QSqlTableModel()
        tableview.setModel(self.model)
        self.model.setTable('PUBLISHER')
        ...
        self.model.setHeaderData(1, QtCore.Qt.Horizontal, "Publisher")
        self.model.setHeaderData(2, QtCore.Qt.Horizontal, "Stars")


    #----------------------------------------------------------
    def print_data(self):
        try:
            sqlite_file = 'PUBLISHERS.db'
            conn = sqlite3.connect(sqlite_file)
            
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM 'PUBLISHER' ORDER BY ID")     # will raise exception if db has not been created
            all_rows = cursor.fetchall()
            pprint(all_rows)
            
            conn.commit()       # not needed when only selecting data
            conn.close()        
        except Exception as ex:             # if the db does not exist
            print(ex)
    ...
    def create_DB(self):
        ...
        query.exec_("create table PUBLISHER(ID int primary key, "
                    "PUBLISHER_NAME varchar(20), PUBLISHER_RATING varchar(20))")
        query.exec_("insert into PUBLISHER values(1000, 'Packt', '1 million stars')")
        query.exec_("insert into PUBLISHER values(1001, 'Amazon', '500 stars')")
        query.exec_("insert into PUBLISHER values(1002, 'Oh, Really?', '1 star')")
        query.exec_("insert into PUBLISHER values(1003, 'Adds & Webs', 'no stars...')")
